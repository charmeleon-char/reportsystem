//------------------------------------------------------------------------------
// <auto-generated>
//    This code was generated from a template.
//
//    Manual changes to this file may cause unexpected behavior in your application.
//    Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace MvcApplication2
{
    using System;
    using System.Collections.Generic;
    
    public partial class V_Class
    {
        public int Schid { get; set; }
        public string Schname { get; set; }
        public Nullable<short> Cycles { get; set; }
        public Nullable<short> Units { get; set; }
        public bool AutoClass { get; set; }
        public Nullable<short> BeginDay { get; set; }
        public Nullable<int> Timeid { get; set; }
        public string Timename { get; set; }
        public string Intime { get; set; }
        public string Outtime { get; set; }
        public string BIntime { get; set; }
        public string EIntime { get; set; }
        public string BOuttime { get; set; }
        public string EOuttime { get; set; }
        public Nullable<short> Latetime { get; set; }
        public Nullable<short> Leavetime { get; set; }
        public Nullable<float> WorkDays { get; set; }
        public Nullable<short> Longtime { get; set; }
        public bool MustIn { get; set; }
        public bool MustOut { get; set; }
        public bool IsFreetime { get; set; }
        public bool IsOvertime { get; set; }
    }
}
